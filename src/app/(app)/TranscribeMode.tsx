...
export default function TranscribeMode() {
	const { getDirectionColor, setTranscribeSettings, transcribeCards, transcribeSettings } =
		useLilacModeRuntime()
	...
	return (
		<div className="flex w-full max-w-5xl flex-col gap-4">
			...
			>
				{transcribeCards.length ? (
					<div className="flex flex-col gap-3">
						{transcribeCards.map(card => (
							<article
								key={card.id}
			...
			<div className="rounded-3xl border border-white/25 bg-[var(--lilac-elevated)]/80 p-4 shadow-lg backdrop-blur">
				<label className="flex flex-col gap-2 md:max-w-sm">
					...
					<select
						...
						onChange={event => setTranscribeSettings({ targetLanguageCode: event.target.value })}
						value={transcribeSettings.targetLanguageCode}
					>
